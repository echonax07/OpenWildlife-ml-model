...
def plot_coco_image(gt_path, pred_path, img_folder, save_folder, mode='file', image_name=None, score_threshold=0.5, iou_threshold=0.5, show_text=True):
    ...
    # Predefined distinct color palette (6 colors, BGR format for OpenCV)
    predefined_palette = [
        (255, 0, 0),    # Blue - Female
        (0, 255, 0),    # Green - Male 
        (0, 0, 255),    # Red
    ...
    category_colors = {
        cat_id: predefined_palette[i % len(predefined_palette)]
        for i, cat_id in enumerate(category_id_to_name.keys())
    }
    ...
    for img_name in tqdm(images_to_process):
        ...
        for category_id, anns in gt_anns.items():
            color = category_colors.get(category_id, (255, 255, 255))  # Same color as predictions for this category
            gt_category_name = category_id_to_name.get(category_id, "Unknown")
            for ann in anns:
                label = f"GT: {gt_category_name}"
                draw_bbox(img, ann['bbox'], color, label, show_text)
...
# # Example usage
# gt_json_path = '/home/m32patel/projects/rrg-dclausi/wildlife/datasets/Eider_duck_labelstudio-export/eider_duck_synth_images/aerial-duck-counting/synthesized_combined/circular/annotations.json'

# pred_json_path = 'work_dirs/mm_grounding_dino_circular_epoch10/prediction_val.bbox.json'
# img_folder = '/home/m32patel/projects/rrg-dclausi/wildlife/datasets/Eider_duck_labelstudio-export/eider_duck_synth_images/aerial-duck-counting/synthesized_combined/circular/'
# save_folder = "/home/m32patel/projects/def-dclausi/whale/mmwhale2/result_viz/eider/mm_grounding_dino_circular_epoch10"
# score_threshold = 0.4
...
save_folder = "/home/m32patel/projects/def-dclausi/whale/mmwhale2/result_viz/eider/mm_grounding_dino_real_filtered_epoch10/real_nonoverlapping_patches"
score_threshold = 0.4
