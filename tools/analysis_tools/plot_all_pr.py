...
def calculate_metrics(gt_json_path, pred_json_path, score_threshold=0.5, image_ids=None, distance_threshold=10):
    """
    ...
    false_positives = 0
    false_negatives = 0
    images_with_predictions_only = 0
    fp_in_images_with_predictions_only = 0
    ...
    for img_id in image_ids:
        ...
        pred_boxes = [ann['bbox']
                      for ann in anns_pred if ann['score'] >= score_threshold]

        if len(gt_boxes) == 0 and len(pred_boxes) > 0:
            images_with_predictions_only += 1
            fp_in_images_with_predictions_only += len(pred_boxes)
    ...
    return true_positives, false_positives, false_negatives, images_with_predictions_only, fp_in_images_with_predictions_only
...
for GT, pred, name in zip(GT_json, pred_json, names):
    try:
        len_anns, cur, r, p, score, auc = plot_confusion_matrix(GT, pred)
        # Find the index of the element closest to 0.9
        closest_index = np.argmin(np.abs(r - 0.9))
        score_at_p_90 = score[closest_index]
        tp, fp, fn, images_with_predictions_only, fp_in_images_with_predictions_only = calculate_metrics(
            GT, pred, score_threshold=score_at_p_90, distance_threshold=1000)
        print(
        ...
        experiment_data = {
            ...
            "p2": tp/(tp+fp),
            "r2": tp/(tp+fn),
            "no_GT_only_pred": images_with_predictions_only,
            "fp_in_images_with_predictions_only": fp_in_images_with_predictions_only
        }
        ...
        if len_anns == 0:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    customdata=f1,  # Add f1 as custom data
                    hovertemplate='Pre: %{y:.3f}<br>' +
                    'Rec: %{x:.3f}<br>' +
                    'F1: %{customdata:.3f}<br>' +  # Use customdata for F1
                    'Score: %{text:.3f}<br>' +
                    f'FP: {fp}<br>' +
                    f'FN: {fn}<br>' +
                    f'r2: {tp/(tp+fn)}<br>' +
                    f'p2: {tp/(tp+fp)}<br>' +
                    f'no_GT_only_pred: {images_with_predictions_only}<br>' +
                    f'fp_in_images_with_predictions_only: {fp_in_images_with_predictions_only}<br>' +
