#!/usr/bin/env python3
"""
Fetch Label Studio annotations via API and convert to COCO format.
...
IMAGE_FILTER_STRINGS = [
    "8776", "8765", "8721", "8716", "8711", "8722", "8613", "8586",
    "8483", "7486", "4655", "4623", "4616", "4611", "4571", "4570",
    "4566", "4564", "4506", "4500", "4369", "4352", "4232", "4219",
    "4197", "4128", "4119", "4109"
]

# Output directory and file
OUTPUT_DIR = 'human_annotations_coco'
...
class LabelStudioToCOCO:
    def __init__(self, output_dir: str):
        ...
        self.coco_data = {
            "info": {
                "description": "Filtered duck annotations from Label Studio",
                "url": LS_URL,
                "version": "1.0",
    ...
    def process_task(self, task: Dict) -> bool:
        """
        Process a single Label Studio task and add to COCO format.
        ...
        if not task.get('annotations') or not task['annotations']:
            print(f"Task {task['id']} ({filename}): No annotations found")
            return False
        ...
        if not results:
            print(f"Task {task['id']} ({filename}): Empty annotation results")
            return False
        ...
        # Extract bbox dimensions from textarea annotations
        ...
        print(f"Task {task['id']} ({filename}): Processed {keypoint_count} annotations")
        return True

    def fetch_and_convert(self, output_file: str):
        """Main function to fetch tasks from Label Studio and convert to COCO format"""
        ...
        try:
            ...
            print(f"Total annotations in COCO format: {len(self.coco_data['annotations'])}")

            # Save COCO format JSON
            print(f"\nSaving COCO annotations to: {output_file}")
...
def main():
    ...
    converter = LabelStudioToCOCO(OUTPUT_DIR)
