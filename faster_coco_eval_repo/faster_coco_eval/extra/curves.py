...
class Curves(ExtraEval):
    def build_curve(self, label):
        ...
        for category_id in cat_ids:
            ...
            auc = round(COCOeval_faster.calc_auc(
                recall_list, precision_list), 4)
    ...
    def plot_pre_rec(self, curves=None, plotly_backend=False, label="category_id", dataset='', show=True):
        if curves is None:
        ...
        for _curve in curves:
            ...
            if use_plotly:
                ...
                axes[0].set_title(f'Precision-Recall: {dataset}')
        ...
        return fig, recall_list, precision_list, scores, name
