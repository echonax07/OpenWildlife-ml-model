...
def slice_image(
    ...
    def _export_single_slice(image: np.ndarray, output_dir: str, slice_file_name: str, save: bool):
        if save:
            image_pil =  image
    ...
    image_height, image_width, _ = image_pil.shape
    if not (image_width != 0 and image_height != 0):
        raise RuntimeError(f"invalid image size: {image_pil.shape} for 'slice_image'.")
    ...
    for slice_bbox in slice_bboxes:
        ...
        if len(sliced_coco_annotation_list) == 0:
           is_postive_sample_list.append(False)
        else:
           is_postive_sample_list.append(True)
